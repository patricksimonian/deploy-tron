...
import {
  ...
  deploymentsForRef,
  deploymentStatusForDeploymentWhereAllSuccessful,
...
describe('As a user I can ask the bot to deploy my microservice', () => {
  ...
  test('When I ask to deploy to dev but the deployment command is malformed, it does not create the deployment', async () => {
    const prComment = replaceCommentBodyWithCommand(
      pullRequestComment,
      'deploy invalidservice to prod',
    );
    const {
      issue: {
        user: { login: user },
    ...
    expect(mock.activeMocks()).toStrictEqual([
      `GET https://api.github.com:443/repos/${owner}/${repo}/deployments`,
    ]);
  });
  ...
  test('When I ask to deploy but there is already a pending deployment to that environment it does not create the deployment', async () => {
    ...
    const statusesRegExp = new RegExp(
      `/repos/${owner}/${repo}/deployments/[0-9]+/statuses`,
    );
    ...
    const mock = nock('https://api.github.com')
      ...
      .persist()
      .get(
        `/repos/${owner}/${repo}/deployments\?ref=${deploymentsForRef[0].ref}`,
      )
