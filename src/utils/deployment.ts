...
export const createDeployment = (
  ...
  return context.octokit.repos.createDeployment({
    repo,
    owner,
    ref,
    environment,
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const isTherePendingDeploymentForEnvironment = async (
  context: Context,
  ref: string,
  env: string,
  repo: string,
  ...
  const data = await context.octokit.graphql(LATEST_STATUS_QL_QUERY, {
    repo,
    owner,
  ...
  return data.repository.deployments.edges.filter((edge: LatestStatus) => {
    ...
    return (
      (hasRef &&
        edge.node.latestStatus === 'PENDING' &&
        edge.node.ref.name !== ref) ||
      (hasRef &&
        edge.node.latestStatus === 'IN_PROGRESS' &&
        edge.node.ref.name !== ref) ||
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getLatestEnvironmentStatusesForRef = async (
  context: Context,
  ref: string,
  repo: string,
  owner: string,
): Promise<any> => {
  const response = await context.octokit.repos.listDeployments({
    repo,
    owner,
    ref,
  });
  ...
  const groupedDeployments = response.data.reduce(
    (groupedDeployments: DeploymentGroup, deployment: Deployment) => {
  ...
  const latestStatuses = Object.keys(groupedDeployments).map(
    async (env: string) => {
      return {
        env,
        status: await context.octokit.repos.listDeploymentStatuses({
          repo,
          owner,
          deployment_id: groupedDeployments[env].id,
          per_page: 1,
        }),
      };
    },
 ...
 * @returns {boolean}
 */
export const isEnvironmentAllowedToDeploy = (
  requiredEnvironments: string[],
  deploymentStatuses: DeploymentStatusGroup,
): boolean => {
  if (!requiredEnvironments || requiredEnvironments.length === 0) return true;

  return requiredEnvironments.every(
    (env) =>
      deploymentStatuses[env] && deploymentStatuses[env].state === 'success',
  );
};
